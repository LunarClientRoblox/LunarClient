...
local specteamexploit = {Enabled = false}
specteamexploit = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
    Name = "Spectate Team Exploit",
    Function = function(callback)
        if callback then 
            task.spawn(function()
                pcall(function()
                    task.spawn(function()
                        ...
                        game:GetService("ReplicatedStorage").TS.remotes.Client:Get("GameStart"):Connect(function()
                            gameStarted = true
                        end)
                        repeat task.wait() until gameStarted
                    end)
                end)
            end)
        end
